...
fn extended_dependency_namespaces(
    metadata: &CargoMetadata,
    seed_ids: &HashSet<String>,
) -> Result<HashMap<String, Vec<String>>, Error> {
    let dependency_nodes: HashMap<_, _> = metadata
        .resolve
        .as_ref()
        .map(|resolve| {
            resolve
        ...
        .unwrap_or_default();
    let mut namespaces = HashMap::<String, Vec<String>>::new();

    for package in metadata
        .packages
        .iter()
        .filter(|package| seed_ids.contains(&package.id))
    {
        let Some(root) = Path::new(&package.manifest_path).parent() else {
            continue;
        };
        let mut targets = HashSet::new();
        iter::iter_crate(&root.to_string_lossy(), |_, source| {
            targets.extend(extractor::extend_targets(source));
            Ok(())
        })?;

        let Some(node) = dependency_nodes.get(package.id.as_str()) else {
            continue;
        };
        for dependency in &node.deps {
            if targets.contains(&dependency.name.replace('-', "_")) {
                namespaces
    ...
    for values in namespaces.values_mut() {
        values.sort();
        values.dedup();
    }
    Ok(namespaces)
...
fn discover_scan_roots(
    ...
    let dependency_namespaces = extended_dependency_namespaces(&metadata, &seed_ids)?;
    ...
    for package in metadata.packages {
        ...
        let namespaces = dependency_namespaces.get(&package.id);
        let namespaces: Vec<Option<String>> = namespaces
            .map(|namespaces| namespaces.iter().cloned().map(Some).collect())
            .unwrap_or_else(|| vec![None]);

        for namespace in namespaces {
            let key = (path_key(&root), namespace.clone());
            if seen.insert(key) {
...
fn i18n(args: I18nArgs) -> Result<(), Error> {
    ...
    for scan_root in scan_roots {
        let root = scan_root.root.to_string_lossy().to_string();
        iter::iter_crate(&root, |path, source| {
            extractor::extract_with_namespace(
                &mut results,
                path,
                source,
                cfg.clone(),
                scan_root.namespace.as_deref(),
            )
        })?;
    }
...
fn convert_text(
    ...
    match format {
        ...
        "yaml" | "yml" => {
            let text = serde_saphyr::to_string(&value)?;
            // Remove leading `---`
