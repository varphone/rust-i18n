...
This mode resolves the Cargo dependency graph and recursively scans workspace members, local path dependencies, and registry crates that depend on `rust-i18n`. The extracted keys are still merged into the root project's `load-path`. Dependencies passed to `extend!(dependency_alias)` are emitted below that alias namespace, so `Widget.title` becomes `dependency_alias.Widget.title`. Because it may scan third-party source trees, it can be slower and may pull dependency texts into your root locale files.
